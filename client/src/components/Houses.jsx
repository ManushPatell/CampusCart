...
const Houses = () => {
  ...
  const filteredListings = mockListings.filter((house) => { // filters the mock listings based on search term and selected filters
    const matchesSearch =
    ...
    const matchesUtilities =
      house.details.utilities.toLowerCase().includes(filters.utilities.toLowerCase()); 
  ...
  return (
    ...
    <div className="min-h-screen bg-[#F5F1EA] font-serif">
      ...
      <div className="py-8 px-4">
        <h1 className="text-4xl font-extrabold text-[#4A4032]">Housing Listings</h1>
      </div>
      ...
      <div className="px-4 py-6">
        <h2 className="text-2xl text-[#4A4032] font-serif">
          Campus Cart does not guarantee all content in the classifieds section to be true.
          Duplicate and/or multiple listings for the same content will be deleted.
        </h2>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {['Any', '1', '2', '3', '4', '5'].map((count) => (
          <button
            key={count} 
            onClick={() =>
              setFilters((prev) => ({// updates the filters state with the selected number of bedrooms
                ...prev,
