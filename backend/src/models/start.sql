...
CREATE TABLE "textbooks" ("id" UUID PRIMARY KEY DEFAULT gen_random_uuid(), "book_title" varchar(255) not null, "author" varchar(255), "edition" varchar(255), "seller" UUID REFERENCES users(id) ON DELETE CASCADE,"date_posted" DATE DEFAULT CURRENT_DATE, "photos" BYTEA[] not null, "year" year, "faculty" varchar(255), "price" INT not null);
